...
在 ROS 2 里，话题就像一个广播频道，用来让不同的程序模块（称为节点）互相传递信息。一个节点可以向某个话题“发消息”，这一动作称为发布；另一个节点可以“收听”这个话题，这一动作称为订阅，这样它们就能共享数据。

话题通信不仅可以是一对一的（你说我听），还可以是一对多、多对一、多对多。此外，一个节点也可以订阅自己发布的话题。
一对一：
...
## 创建话题接口
我们创建话题接口的方式和创建功能包的方式一样，话题接口本质上也是一种功能包，只不过它只包含一类文件——消息文件。话题的本质就是数据结构，在C/C++中，我们可以用结构体来定义数据结构，如下所示：
...
在工作空间的src目录下，输入以下命令创建一个名为address_book的包，使用ament_cmake编译方式，依赖rclcpp和more_interfaces。一般来说，如果一个节点要使用自定义消息接口，就需要为它添加对应的依赖。
...
在 address_book/src 下创建 publish_address_book.cpp、subscribe_address_book.cpp 两个文件，并修改CMakeLists.txt、package.xml文件。
