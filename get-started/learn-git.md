...
再在上方的输入框里写上这个commit中更改的内容（即commit message），点击提交，
...
再点整个界面左下角的刷新图标（这里可能略有不同，会多几个数字），推送刚才的commit到服务器，这样就成功把你想要提交的文件传到git仓库里了

![](/push.png)

::: info 说明
也可以点右上角`源代码管理`这一栏三个点，再点击`拉取，推送 - 推送`

注意这里有两个源代码管理，而且刚好都有下拉选项，要点下面一行的鼠标移过去才会显示的那个
:::
